...
public class ExactBaseline {
    ...
    public ExactBaseline(int graph_size, int degree, int dimension, double range, int hotels_num, int obj_dimension) {
        this.range = range;
            ...
            for (Map.Entry<Long, myNode> entry : tmpStoreNodes.entrySet()) {
                ...
                for (path p : my_n.skyPaths) {
                    if (!p.rels.isEmpty()) {
                        ...
                        //without the index, the candidate node list is the set of POI objects that is not dominated by query points
