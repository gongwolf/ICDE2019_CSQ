...
Each graph edge represents a segment in a bus line with **three**-dimensional costs. The raw data only contains the first cost value is the *distance* between two end nodes of a edge. Other two cost values are generated by sampling from a uniform distribution based on the distance. 
